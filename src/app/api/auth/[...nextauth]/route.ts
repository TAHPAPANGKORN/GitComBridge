...
export const authOptions: NextAuthOptions = {
  ...
  events: {
    async linkAccount({ account }) {
      console.log("🔗 Linking account for provider:", account.provider);
      try {
        // When an account is linked, encrypt the tokens if possible
        if (account.access_token && (ENCRYPTION_KEY.length === 32 || ENCRYPTION_KEY.length === 64)) {
          ...
          console.warn("⚠️ Skip encryption: Missing access_token or invalid ENCRYPTION_KEY length.");
        }
      } catch (error) {
