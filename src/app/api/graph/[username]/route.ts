...
import { generateSVG, ThemeName, CellSize, GraphLayout, AnimationType, ViewMode, ShapeType } from "@/lib/utils/svg-generator";
...
export async function GET(
  ...
  const requestedView    = (searchParams.get("viewMode") as ViewMode) || "flat";
  const requestedShape   = (searchParams.get("shape") as ShapeType) || "square";
  ...
  try {
    ...
    const svg = generateSVG(
      ...
      { 
        ...
        animation: requestedAnim, viewMode: requestedView, shape: requestedShape 
      },
      userTier
    );
